...
This reference sheet includes vital information for programmers. This document is \textbf{Java-centric}.

\noindent \textbf{Contact Miraj Parikh if you have any questions or if you need any clarification on this reference sheet.}

% \section{Glossary}
%
% \begin{center}\begin{tabularx}{\textwidth}{lcX}\toprule
%   Term & Part of Speech & Description \\\midrule
%   Boolean     & n. & True-or-False: \\\midrule
%   Assignment  & a. & Compound assignment operators\\\midrule
% \end{tabularx}\end{center}

\renewcommand{\arraystretch}{1.2} % when the numbers are too close together it gives me a headache
\section{Primitive Data Types (Java \& C-family)}
Exceptions: The C-family \lstinline|char| primitive actually has the same dimensions as a byte.
For Java, all primitives are \textit{signed}.
\begin{center}\begin{tabular}{lllr}\toprule
  Name {\small (Java Wrapper Class)}      &Size (Bits) &Initialization              &Range \\\midrule
...
% String
\multirow{7}{*}{String} & \lstinline|x.capitalize()| & Return string with first letter capitalized\\
...
\multirow{3}{*}{General} & \lstinline|x.isEmpty()| & Returns a boolean indicating whether the container \(x\) is empty\\
...
& \lstinline|x.substring(i, j)| & Return a string of characters in \(x\) from index \(i\) to before index \(j\) (inclusive-exclusive); if \(j\) is not given, then \(j\) will equal the length, or \lstinline|x.length()|\\\midrule
...
% HashMap
\multirow{7}{*}[-0.5em]{\lstinline|HashMap|} & \lstinline|Map<T, U> x = new HashMap<>();| & Initialize a HashMap \(x\) mapping \(T\)-typed to \(U\)-typed objects\\
...
% HashSet
\multirow{4}{*}{\lstinline|HashSet|} & \lstinline|Set<T> x = new HashSet<>();| & Initialize a HashSet \(x\) holding \(T\)-typed objects\\
...
\section{Other}
\subsection{ASCII and Conversions}
\subsubsection{Chart of Relevant Characters}
...
40 & 28 & \texttt{(} & 59 & 3B & \texttt{;} & 78 & 4E & \texttt{N} & \textbf{97} & \textbf{61} & \texttt{a} & 116 & 74 & \texttt{t} \\
...
46 & 2E & \texttt{.} & \textbf{65} & \textbf{41} & \texttt{A} & 84 & 54 & \texttt{T} & 103 & 67 & \texttt{g} & 122 & 7A & \texttt{z} \\
47 & 2F & \texttt{/} & 66 & 42 & \texttt{B} & 85 & 55 & \texttt{U} & 104 & 68 & \texttt{h} & 123 & 7B & \texttt{\{} \\
\textbf{48} & \textbf{30} & \texttt{0} & 67 & 43 & \texttt{C} & 86 & 56 & \texttt{V} & 105 & 69 & \texttt{i} & 124 & 7C & \texttt{|} \\
...
\bottomrule\end{tabular}\end{center}
\subsubsection{ASCII tricks}
In C-family languages, a character which represents a number (i.e., \lstinline|'8'|) can be substracted by \lstinline|'0'| to return its actual value.
For example, in C++ with a defined string \lstinline|s| with an index \(i\), this would be \lstinline|s[i]-'0'|; and in Java,  \lstinline|s.charAt(i)-'0'|.
...
\begin{tikzpicture}
    ...
    \node[anchor=south,align=center] at (current bounding box.north)
      {Intersection\\\(A \cap B\)\\\texttt{A AND B}\\\lstinline|x&y|};
\end{tikzpicture}
\qquad
\begin{tikzpicture}
    ...
    \node[anchor=south,align=center] at (current bounding box.north)
      {Symmetric Difference\\\(A \oplus B\)\\\texttt{A XOR B}\\\lstinline|x^y|};
\end{tikzpicture}
\qquad
\begin{tikzpicture}
    \draw[filled]
      \firstcircle node {\(A\)} \secondcircle node {\(B\)};
    \node[anchor=south,align=center] at (current bounding box.north)
      {Union\\\(A \cup B\)\\\texttt{A OR B}\\\lstinline+x|y+};
\end{tikzpicture}
\end{center}
...
%\enquote{When in doubt, add parentheses to random expressions thinking it might do something} - George Washington
